...
filecod<-read_json(filename,simplifyVector = TRUE)
save(filecod,file="data/FileCodificado.RData")
```
...
```{r}
# ...
write_json(iris,path = "data/iris.json")
iris2<-read_json("data/iris.json")
#me sale vacio
```
...
# Leer sin tipos definidos para explorar
df_preview <- read_excel(fname, sheet = 2)
print("Estructura inicial:")
str(df_preview)

# Definir tipos de columna 
ct <- c(
  "text", "date", "date", "numeric", "text", "text", "text",
  rep("numeric", 51)
)

# Leer con tipos definidos
df <- read_excel(fname, sheet = 2, col_types = ct)

# 2. LIMPIEZA
df_clean <- df %>% 
  filter(!is.na(IDENTIFICA)) %>% 
  filter(complete.cases(IDENTIFICA))  # Filas con ID válido
...
# 4. GUARDAR
save(df_clean, file = "data/ERCA_clean.RData")
print("Datos guardados en data/ERCA_clean.RData")
```
...
# Obtener y mostrar hojas disponibles
sheets <- excel_sheets(filename)
cat("Hojas encontradas:", paste(sheets, collapse = ", "), "\n")
...
# Guardar
save(df_list, file = "data/subjectInfo_completo.RData")
cat("Datos guardados en subjectInfo_completo.RData\n")

# 7.2 INSPECCIÓN
cat("\n=== INSPECCIÓN DE DATOS ===\n")
for (i in 1:length(df_list)) {
  cat("\n» Hoja:", names(df_list)[i], "\n")
  cat("  Dimensiones:", dim(df_list[[i]]), "\n")
  cat("  Estructura:\n")
  str(df_list[[i]], max.level = 1, vec.len = 2)
}
```
